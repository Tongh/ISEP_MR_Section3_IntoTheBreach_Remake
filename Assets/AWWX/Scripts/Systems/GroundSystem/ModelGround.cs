...
namespace OutOfTheBreach
{
    public interface IModelGround : IModel
    {
        ...
        // 0: no flag, 1: attcked, 2: new monster
        BindableProperty<int>[,] FlagMap { get; }
    }
    ...
    public class ModelGround : AbstractModel, IModelGround
    {
        ...
        public BindableProperty<int>[,] FlagMap { get; } = new BindableProperty<int>[8, 8];
        ...
        private void ResetMap()
        {
            for (int i = 0; i < 8; i++)
            {
                for (int j = 0; j < 8; j++)
                {
                    ...
                    FlagMap[i, j] = new BindableProperty<int>() { Value = 0 };
                }
            }
        }
    }
